...
\begin{frame}{Linear Regression Intuition}
    ...
    \begin{minipage}[c][3\baselineskip][c]{\textwidth}
        \begin{equation*}
            \log\frac{s_{jt}}{s_{0t}} \alt<3->{\approx}{=} \alert<2>{\beta x_{jt}} \only<3->{+ \alert<3-6>{\tfrac{\sigma^2}{2} d_{jt}^x} \only<7->{+ \alert<7>{\pi m_t^y x_{jt}} + \alert<8>{\tfrac{\pi^2}{2} v_t^y d_{jt}^x}}} + \xi_{jt}
        \end{equation*}
    \end{minipage}
    \vspace{-0.5\baselineskip}
