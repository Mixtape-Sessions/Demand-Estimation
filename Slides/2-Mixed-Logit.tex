...
\begin{frame}{Random Coefficients}
    ...
    \begin{minipage}[c][4\baselineskip][c]{\textwidth}
        \only<1-2>{
            \begin{equation*}
                u_{ijt} = x_{jt}'\alt<1>{\beta}{\alert<2>{\beta_{it}}} + \xi_{jt} + \varepsilon_{ijt}
            \end{equation*}
        }
        \only<3->{
            \begin{equation*}
...
\begin{frame}{Linear Regression Intuition}
    ...
    \begin{minipage}[c][4\baselineskip][c]{\textwidth}
        \begin{equation*}
            \log\frac{s_{jt}}{s_{0t}} \approx \alert<2>{\beta x_{jt}} \only<3->{+ \alert<3-6>{\sigma^2 d_{jt}^x} \only<7->{+ \alert<7>{\pi m_t^y x_{jt}} + \alert<8>{\pi^2 v_t^y d_{jt}^x}}} + \xi_{jt} \only<3->{\quad\text{where}\quad d_{jt}^x = \Big(\frac{x_{jt}}{2} - \sum_{k \in \mathcal{J}_t} s_{kt} x_{kt}\Big) x_{jt}}
        \end{equation*}
    \end{minipage}
    \vspace{-0.5\baselineskip}
    \begin{wideitemize}
        ...
        \pause
        \item To target $\sigma$, need a measure of how ``differentiated'' $j$ is in terms of $x_{jt}$ within $t$.
        \only<3-6>{
            \begin{itemize}
        ...
        \pause
        \item To target $\pi$, we can interact $x_{jt}$ with mean within-market income $m_t^y$.
        \only<7-8>{
            \begin{itemize}
