...
\begin{frame}{Nonlinear Optimization}
    ...
    \pause
    \begin{wideitemize}
        \only<3->{\vspace{-0.11\baselineskip}}
        ...
        \pause
        \only<4->{\vspace{-0.11\baselineskip}}
        ...
        \pause
        \only<5->{\vspace{-0.11\baselineskip}}
        ...
        \pause
        \only<6->{\vspace{-0.12\baselineskip}}
        \item Terminate on \alert{strict first-order conditions}, e.g.\ $\lVert\text{gradient}\rVert_\infty < $ \texttt{1e-8}.
        \only<5>{
            \begin{itemize}
                \item Inner loop should be tighter to prevent error ``bubbling up.'' PyBLP's default is very tight.
...
\begin{frame}{Numerical Integration}
    ...
    \vspace{-0.5\baselineskip}
    \pause
    \only<4->{\vspace{0.15\baselineskip}}
    \only<5->{\vspace{-0.1\baselineskip}}
    \only<6->{\vspace{-0.05\baselineskip}}
    \begin{wideitemize}
        \item Individual types $i$ are typically an \textit{approximation} to a population distribution.
        \pause
        \only<4->{\vspace{0.08\baselineskip}}
        \only<5->{\vspace{-0.13\baselineskip}}
        \only<6->{\vspace{-0.07\baselineskip}}
        ...
        \pause
        \only<5->{\vspace{-0.24\baselineskip}}
        \only<6->{\vspace{-0.06\baselineskip}}
        ...
        \pause
        \only<6->{\vspace{-0.17\baselineskip}}
...
\begin{frame}{Linear Regression Intuition}
    ...
    \begin{minipage}[c][3\baselineskip][c]{\textwidth}
        \begin{equation*}
            \log\frac{s_{jt}}{s_{0t}} \alt<3->{\approx}{=} \alert<2>{\beta x_{jt}} \only<3->{+ \alert<3-8>{\tfrac{\sigma^2}{2} d_{jt}^x} \only<9->{+ \alert<9-10>{\pi m_t^y x_{jt}} + \alert<11>{\tfrac{\pi^2}{2} v_t^y d_{jt}^x}}} + \xi_{jt}
        \end{equation*}
    \end{minipage}
    \vspace{-0.5\baselineskip}
    \only<2>{\vspace{0.12\baselineskip}}
    \begin{wideitemize}
        ...
        \pause
        \item To target $\sigma \neq 0$, we need an IV for how ``differentiated'' $j$ is in terms of $x_{jt}$ within $t$:
        \begin{itemize}
            \item $d_{jt}^x = (x_{jt} - \overline{x}_t)^2 - (0 - \overline{x}_t)^2$ where $\overline{x}_t = \sum_{k \in \mathcal{J}_t} s_{kt} x_{kt}$.
            \pause
            \only<4-8>{
                \item Can't use $d_{jt}^x$ itself because it depends on endogenous market shares $s_{kt}$.
                \pause
                \item \defcitealias{berry1995automobile}{BLP}\citetalias{berry1995automobile} IVs from day 1 were the conventional choice: $\sum_{k \neq j} x_{kt}$.
                \pause
                \item \cite{gandhi2025measuring} provide a stronger choice: $\sum_{k \neq j} (x_{jt} - x_{kt})^2$.
                \pause
                \item \cite*{borusyak2025estimating} provide some shift-share alternatives.
                \pause
                \item We want \alert{cross-market choice set variation}, otherwise $d_{jt}^x$ is collinear with $x_{jt}^2$.
            }
        \end{itemize}
        \pause
        \only<10-11>{\vspace{0.0\baselineskip}}
        ...
        \only<10-11>{
            \begin{itemize}
                ...
                \item<11> Can technically identify $\pi$ from higher-order variation, e.g.\ in variance $v_t^y$.
            \end{itemize}
        }
        \only<12>{
